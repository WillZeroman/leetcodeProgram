...
public class Solution {
    public int respace(String[] dictionary, String sentence) {
        Arrays.sort(dictionary, (s1, s2) -> s2.length() - s1.length());
        System.out.println(Arrays.toString(dictionary));
        for (String tmp : dictionary) {
            if (tmp.length() > sentence.length()) {
                continue;
            }
            sentence = sentence.replaceAll(tmp, "");
